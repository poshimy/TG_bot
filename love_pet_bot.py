...
def apply_auto_decay(user_id: int):
    ...
    if delta_hours > 0.05:
        ...
        cleanliness = max(0, int(cleanliness - CLEAN_DECAY_PER_HOUR * delta_hours))
...
async def check_missed_meal(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in user_ids:
        ...
        phrases = MEAL_REMINDER_PHRASES.get(meal_name) or MEAL_REMINDER_PHRASES["default"]
        ...
        try:
            ...
            logger.warning("Не удалось отправить сообщение про штраф пользователю %s: %s", user_id, e)
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, (name, price) in REWARDS.items():
        keyboard.append(
            [InlineKeyboardButton(f"{name} — {price} баллов", callback_data=f"buy_{key}")]
        )

    keyboard.append(
        [InlineKeyboardButton("Проверить баланс", callback_data="shop_balance")]
    )
...
async def rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning("Не удалось отправить сообщение девушке №%s: %s", girl_num, e)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("choose_pet_"):
        ...
        await query.message.reply_text(
            ...
            f"Используй /care, чтобы ухаживать, и /pet, чтобы смотреть статус."
        )
        return
    ...
    if guess == secret_upper:
        ...
        if ADMIN_ID != 0:
            try:
                ...
                logger.warning("Не удалось отправить сообщение админу про квест: %s", e)
